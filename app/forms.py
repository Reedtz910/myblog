...
class RegistrationForm(FlaskForm):
    ...
    def validate_username(self, username):
        ...
        if user is not None:
            raise ValidationError(_('Это имя занято.'))

    def validate_email(self, email):
        user = User.query.filter_by(email=email.data).first()
        if user is not None:
            raise ValidationError(_('Эта почта занята.'))


class EditProfileForm(FlaskForm):
    username = StringField(_l('Имя'), validators=[DataRequired()])
    about_me = TextAreaField(_l('Обо мне'), validators=[
...
class PostForm(FlaskForm):
    post = TextAreaField(_l('Расскажи что-нибудь интересное'), validators=[
...
class ResetPasswordRequestForm(FlaskForm):
    email = StringField(_l('Почта'), validators=[DataRequired(), Email()])
    submit = SubmitField(_l('Запрос восстановления пароля'))
